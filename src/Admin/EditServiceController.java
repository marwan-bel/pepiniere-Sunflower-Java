...
public class EditServiceController {
    ...
    public GestionServices getS() {
        return s;
    }

    public void setS(GestionServices s) {
        this.s = s;
        region();
        Servicesatet.getSelectionModel().selectedItemProperty().addListener((a, b, c) -> {
            choixregion();
        });
        this.s.setId_service(s.getId_service());
        ...
        whichcheck();
        Serviceemail.setText(s.getContact());
        Serviceprice.setText(s.getPrix() + "");
        servicedescrip.setText(s.getDescription());
        ServiceName.setText(s.getNom_service());
        ServiceCity.getSelectionModel().select(s.getRegion());
        servicetype.getSelectionModel().select(s.getType());
        if (s.getEtat().equals("daily")) {
            Daily.setSelected(true);
        }
        if (s.getEtat().equals("weekly")) {
            Monthly.setSelected(true);
        }
        if (s.getEtat().equals("monthly")) {
            Weekly.setSelected(true);
        }
        Image img = new Image("http://localhost/uploads/" + s.getImage());
    ...
    private String whichcheck() {
        ...
        if (Monthly.isSelected()) {
            ...
            alert.setContentText("no check box is selected");
        }
        return l;
    }
    ...
    private void SAjouter(ActionEvent event) throws Exception {
        if (isInputValid()) {
            ...
            sp.modifier(s.getId_service(), s.getNom_service(), s.getPrix(), s.getEtat(), s.getType(), s.getRegion(), s.getDescription(), s.getContact(), s.getImage());
